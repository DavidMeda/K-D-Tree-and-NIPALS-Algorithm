...
int rangeQuery(MATRIX ds, struct kdtree_node *tree, MATRIX qs, int id_qs, float r, int k, int n, int *list, float *point, int *nQA)
{
    ...
    if (dist <= r)
    {
        ...
        // printf(" trovato elemento in lista [%d] = %d ", indexList, list[indexList]);
        indexList++;

        // list[id_qs * n / 2 + indexList] = tree->indexMedianPoint;
        *nQA = *nQA + 1;
    }
    if (tree->left != NULL)
    {
...
void range_query(params *input)
{
    ...
    for (i = 0; i < input->nq; i++)
    {
        ...
        if (indexList == 0)
        {
            // printf("\nlibero spazio per iterazione %d\n", i);
            free(input->QA[i]);
            input->QA[i] = NULL;
        }
        else
        ...
        // input->QA[i * input->n / 2 + indexList] = -1;
        indexList = 0;
    }
    free_block(point);
}
